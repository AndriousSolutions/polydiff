...
class ChatMuteService extends ChangeNotifier {
  ...
  ValueNotifier<Map<String, List<String>>> mutedPlayersPerChannelNotifier =
      ValueNotifier({});
  ...
  Future<List<String>> getPlayersInChat(String channelName) async {
    ...
    SocketService.socket
        .emit('requestPlayersInChat', {'channelName': channelName});
  ...
  Future<Map<String, List<String>>> getMutedPlayersInChannel(
      String channelName, String username) async {
  ...
  void unmutePlayer(
      String username, String channelName, String playerToUnmute) {
    ...
    final currentMutedPlayers = mutedPlayersPerChannelNotifier.value;
    final mutedPlayers = currentMutedPlayers[channelName]
            ?.where((player) => player != playerToUnmute)
            .toList() ??
        [];
    currentMutedPlayers[channelName] = mutedPlayers;
