...
class CardQueueService {
  ...
  void setUpImage(dynamic data) async {
    ...
    // List<Difference> differences = await differencesRemoved.stream.first;
    // List<Map<String, dynamic>> serializedDifferences = differenceRemovedList.map((difference) => difference.toJson()).toList();
    // SocketService.socket.emit('removeDifferences', {
    //   'Img1': leftImageBase64,
    //   'Img2': rightImageBase64,
    //   'remove': differencesRemoved
    // }); // a voir comment implemanter les valeurs de diffs, img1 et 2.
